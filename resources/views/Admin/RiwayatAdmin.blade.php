  ...
  <main class="main-content">
    ...
    <div class="p-4 p-lg-5">
      ...
      <div class="admin-info-header shadow-sm">
        <img src="https://ui-avatars.com/api/?name=Admin+Utama&background=0f172a&color=fff" class="rounded-4" width="60">
        <div>
      ...
      <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        ...
        <div class="d-flex gap-2">
            <input type="text" class="form-control form-control-sm border-slate-200 rounded-3 px-3" placeholder="Cari ID Tiket..." style="width: 200px;">
        </div>
      </div>
      <div class="d-flex gap-2 flex-wrap">
        <a href="?range=hari-ini" class="btn btn-sm rounded-pill px-3 {{ request('range') == 'hari-ini' ? 'btn-primary' : 'btn-light border text-muted' }}">Hari Ini</a>
        <a href="?range=kemarin" class="btn btn-sm rounded-pill px-3 {{ request('range') == 'kemarin' ? 'btn-primary' : 'btn-light border text-muted' }}">Kemarin</a>
        <a href="?range=minggu-ini" class="btn btn-sm rounded-pill px-3 {{ request('range') == 'minggu-ini' ? 'btn-primary' : 'btn-light border text-muted' }}">Minggu Ini</a>
        <a href="?range=minggu-lalu" class="btn btn-sm rounded-pill px-3 {{ request('range') == 'minggu-lalu' ? 'btn-primary' : 'btn-light border text-muted' }}">Minggu Lalu</a>
        <a href="?range=bulan-ini" class="btn btn-sm rounded-pill px-3 {{ request('range') == 'bulan-ini' ? 'btn-primary' : 'btn-light border text-muted' }}">Bulan Ini</a>
        <a href="?range=bulan-lalu" class="btn btn-sm rounded-pill px-3 {{ request('range') == 'bulan-lalu' ? 'btn-primary' : 'btn-light border text-muted' }}">Bulan Lalu</a>
          ...
          <div class="log-card">
            <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
              <div class="d-flex align-items-center gap-3">
                ...
                <div>
                   <h6 class="fw-bold mb-0">Memperbarui Tiket <a class=" text-decoration-none" href="{{ url('Admin/DashboardAdmin') }}#aspirasi-231{{ $progres->id_aspirasi }}">#SPR-231{{ $progres->id_aspirasi }}</a></h6>
                   <h6 class="fw-bold mb-0 ml-4 pt-1">{{ $progres->judul_aspirasi }}</h6>
                  <p class="text-muted xsmall mb-0">Dilakukan pada{{ $progres->tanggal_update }}</p>
                </div>
              </div>
              <div class="d-flex align-items-center">
                @switch($progres->status_lama)
                  @case('menunggu')
                     <span class="status-badge bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25">{{ $progres->status_lama }}</span>
                    @break
                  @case('diproses')
                     <span class="status-badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25">{{ $progres->status_lama }}</span>
                    @break
                  @default
                @endswitch
                <i class="fa-solid fa-arrow-right-long change-arrow"></i>
                <span class="status-badge bg-success bg-opacity-10 text-success border border-success border-opacity-25">{{ $progres->status }}</span>
              </div>
            </div>
            ...
            <div class="row mt-4">
              ...
              <div class="col-md-6">
                ...
                <div class="log-details border-start border-4 border-success bg-success bg-opacity-10">
                  {{ $progres->umpan_balik }}
                </div>
              </div>
            </div>
              ...
              <div class="d-flex align-items-center">
                @switch($progres->status_lama)
                  @case('menunggu')
                    ...
                    @break
                  @case('selesai')
                     <span class="status-badge bg-success bg-opacity-10 text-success border border-success border-opacity-25">{{ $progres->status_lama }}</span>
                    @break
                  @case('ditolak')
                    <span class="status-badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25">{{ $progres->status_lama }}</span>
                  @break
                  @default
                  <span class="status-badge bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25">menunggu</span>
                @endswitch
                <i class="fa-solid fa-arrow-right-long change-arrow"></i>
                <span class="status-badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25">{{ $progres->status }}</span>
              </div>
            </div>
              ...
              <div class="d-flex align-items-center">
                @switch($progres->status_lama)
                  ...
                  @case('diproses')
                    ...
                    @break
                  @default
                  <span class="status-badge bg-info bg-opacity-10 text-info border border-info border-opacity-25">baru</span>
                @endswitch
                <i class="fa-solid fa-arrow-right-long change-arrow"></i>
                <span class="status-badge bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25">{{ $progres->status }}</span>
              </div>
            </div>
            <div class="row mt-4">
              <div class="col-12">
                <span class="log-label">Alasan Penolakan</span>
                <div class="log-details border-start border-4 border-warning bg-warning bg-opacity-10">
                  {{ $progres->ket_progres }}
                </div>
              </div>
            </div>
           ...
           <div class="log-card">
            <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
              ...
              <div class="d-flex align-items-center">
                ...
                @endswitch
                <i class="fa-solid fa-arrow-right-long change-arrow"></i>
                <span class="status-badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25">{{ $progres->status }}</span>
              </div>
            </div>
            ...
            <div class="row mt-4">
              <div class="col-12">
                <span class="log-label">Alasan Penolakan</span>
                <div class="log-details border-start border-4 border-danger bg-danger bg-opacity-10">
                  {{ $progres->ket_progres }}
                </div>
              </div>
            </div>
