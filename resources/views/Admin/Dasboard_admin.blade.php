...
<head>
  ...
  <style>
    ...
    /* ===============================
       MODAL CUSTOM
       =============================== */
    .modal-custom .bg-soft-warning { background-color: rgba(255, 193, 7, 0.1); }
    ...
    .modal-custom .timeline-item:last-child { border-left-color: transparent; }
    ...
    .modal-custom .btn-check:checked + .btn-outline-primary { background-color: #0d6efd; color: #fff; }
    .modal-custom .btn-check:checked + .btn-outline-success { background-color: #198754; color: #fff; }
    .modal-custom .btn-check:checked + .btn-outline-danger { background-color: #dc3545; color: #fff; }
...
@media (max-width: 991.98px) {
    ...
    #menu-control:checked ~ .sidebar-overlay { display: block; }
}

/* Efek saat baris 'dituju' dari link riwayat */
tr:target {
    background-color: rgba(124, 58, 237, 0.1) !important; /* Warna ungu pudar sesuai tema purple kamu */
...
.btn-reset {
    ...
    cursor: pointer;
    outline: inherit;
    width: 100%; /* Biar bisa diklik sepanjang baris dropdown */
    text-align: left; /* Biar teksnya tetep di kiri kayak link biasa */
}
  </style>
...
<body>
  ...
  <main class="main-content">
    <header class="top-bar">
      ...
      <div class="d-flex align-items-center gap-3">
        ...
        <div class="profile-trigger d-flex align-items-center gap-3" data-bs-toggle="dropdown" aria-expanded="false">
          ...
          <img src="https://ui-avatars.com/api/?name=Admin&background=7c3aed&color=fff" class="rounded-circle shadow-sm" width="40">
        </div>
    ...
    <div class="p-4 p-lg-5">
      ...
      <div class="card border-0 shadow-sm rounded-4 mb-4">
          <div class="card-body p-4"> <!-- Padding dalam kartu sebesar level 4 -->
              ...
              <form action="{{ url('Admin/DashboardAdmin/Filter') }}" method="GET" class="row g-3 align-items-end">

                  <!-- Kolom Input Pencarian Siswa (Lebar 3/12 pada layar medium) -->
                  <div class="col-md-3">
                      <label class="form-label small fw-bold text-muted"><i class="fa-solid fa-user me-1"></i> Cari Siswa</label>
                      <!-- Input teks dengan fitur 'list' yang terhubung ke datalist id="siswaOptions" -->
                      <input class="form-control rounded-3 border-slate-200" list="siswaOptions" id="siswaFilter" name="siswa" placeholder="Ketik nama siswa...">

                      <!-- Daftar pilihan otomatis (autocomplete) yang diambil dari variabel $allSiswa -->
                      <datalist id="siswaOptions">
                          @foreach($dataSiswa as $s)
                              <option value="{{ $s->id_siswa }}">{{ $s->Nama }}</option> <!-- Menampilkan nama siswa sebagai saran ketikan -->
                          @endforeach
                      </datalist>
                  </div>
                  ...
                  <div class="col-md-2">
                      <label class="form-label small fw-bold text-muted"><i class="fa-solid fa-calendar-days me-1"></i> Bulan</label>
                  ...
                  <div class="col-md-3">
                      <label class="form-label small fw-bold text-muted"><i class="fa-solid fa-tags me-1"></i> Kategori</label>
                  ...
                  <div class="col-md-2">
                      <label class="form-label small fw-bold text-muted"><i class="fa-solid fa-calendar-range me-1"></i> Tanggal</label>
                  ...
                  <div class="col-md-2 d-flex gap-2">
                      ...
                      <a href="{{ url('Admin/DashboardAdmin') }}" class="btn btn-light border rounded-3 fw-bold" title="Reset">
        ...
        <table class="table mb-0">
          ...
          <tbody>
            ...
            <tr id="aspirasi-231{{ $data->id_aspirasi }}">
              ...
              <td><span class="fw-bold text-primary">#SPR-231{{ $data->id_aspirasi }}</span></td>
              <td>
    ...
    @default
        ...
        <span class="status-badge bg-info bg-opacity-10 text-info border border-info border-opacity-25">
...
@endswitch
              ...
              <td class="text-center">
                ...
                <button class="action-btn" title="Kelola Aspirasi"
        ...
        <tr>
            <td colspan="5" class="text-center py-5">
                <div class="d-flex flex-column align-items-center">
                    ...
                    <p class="small text-muted">Siswa belum membuat laporan atau Pastikan nama siswa benar.</p>
                </div>
            </td>
        </tr>
    @endforelse
        ...
        <div class="modal-body p-0">
          <div class="row g-0">
            ...
            <div class="col-lg-6 border-end p-4 bg-white">
              ...
              <div class="d-flex align-items-center gap-3 mb-4 p-3 border rounded-4 bg-light bg-opacity-50">
              ...
              <form action="{{ url('Admin/Aspirasi/Tambah') }}" method="post">
                ...
                <div class="mb-4">
                  ...
                  <div class="d-flex flex-wrap gap-2">
                    <select
                        ...
                        class="form-select"
                        required>
                        <option value="" @selected($data->status == '' )>Menunggu Respon</option>
                ...
                <div class="">
                  <label class="form-label fw-bold small">Tanggal Update</label>
                   <input class="form-control border-0 shadow-sm rounded-4 p-3 small" type="date" class="form-control" name="tanggal_update" value="<?= date('Y-m-d') ?>" readonly>
                </div>

                <div class="mb-4">
                  <label class="form-label fw-bold small">Keterangan Progres (Internal)</label>
                  <textarea class="form-control border-0 shadow-sm rounded-4 p-3 small" rows="3" placeholder="Tulis catatan perbaikan di sini..." name="ket_progres" required></textarea>
                </div>
                ...
                <div class="mb-4" id="feedbackContainer" style="display:none;">
                  <div class="p-3 bg-success bg-opacity-10 rounded-4 border border-success border-opacity-25">
                    <label class="form-label fw-bold small text-success"><i class="fa-solid fa-comment-dots me-1"></i> Pesan untuk Siswa</label>
                    <textarea class="form-control border-0 shadow-sm rounded-3 small" rows="2" placeholder="Sampaikan bahwa masalah telah tuntas..." name="umpan_balik"></textarea>
                    <div class="form-text text-success" style="font-size:0.7rem;">Umpan balik ini akan tampil di dashboard siswa.</div>
                  </div>
                </div>
        ...
        <div class="modal-body p-4">
          <div class="text-center mb-4">
            <div class="position-relative d-inline-block">
              ...
              <label class="position-absolute bottom-0 end-0 bg-primary text-white rounded-circle d-flex align-items-center justify-content-center border border-2 border-white" 
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control rounded-3 border-slate-200 bg-light" 
                   value="{{ auth()->guard('admin')->user()->username }}" readonly>
            <div class="form-text text-muted" style="font-size: 0.7rem;">Username tidak dapat diubah.</div>
          </div>

          <div class="mb-3">
            <label class="form-label small fw-bold text-muted">Nama Lengkap</label>
            <input type="text" name="nama" class="form-control rounded-3 border-slate-200" 
                   value="Administrator Utama" required>
          </div>

          <div class="mb-4">
            <label class="form-label small fw-bold text-muted">Jabatan</label>
            <select name="jabatan" class="form-select rounded-3 border-slate-200">
              <option value="Admin" selected>Admin Utama</option>
...
<script>
  ...
  function populateModal(ticketID, ID, studentName, subject, lokasi, status, description, tanggal_update, tglLapor, ket) {
    ...
    const avatarUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(studentName)}&background=7c3aed&color=fff`;
    ...
    switch(status.toLowerCase()) {
      case 'menunggu': badgeClass = 'bg-warning bg-opacity-10 text-warning border border-warning'; break;
      case 'diproses': badgeClass = 'bg-primary bg-opacity-10 text-primary border border-primary'; break;
      case 'selesai': badgeClass = 'bg-success bg-opacity-10 text-success border border-success'; break;
      default: badgeClass = 'bg-secondary bg-opacity-10 text-secondary border border-secondary';
    }
